...
def main(config, checkpoint, ph14_root, ph14_lmdb_root, index):
    ...
    single_loader = torch.utils.data.DataLoader(
        single_dataset, batch_size=1, num_workers=6, collate_fn=dm.collate_fn
    )
    ...
    single_loader = torch.utils.data.DataLoader(
        [single_data], batch_size=1, collate_fn=dm.collate_fn
    )
    ...
    # bgr2rgb
    original_images = np.uint8(
        single_data["video"].numpy().transpose(0, 2, 3, 1) * 255.0
    )
...
def visualize_and_save_grad_cam_images(
    original_images, grad_cam_heatmap_batch, output_dir
):
    ...
    for i, (original_image, grad_cam_heatmap) in enumerate(
        zip(original_images, grad_cam_heatmap_batch)
    ):
        # Resize Grad-CAM heatmap to match the input image size
        grad_cam_heatmap_resized = cv2.resize(
            grad_cam_heatmap, (original_image.shape[1], original_image.shape[0])
        )

        # Apply colormap for visualization
        grad_cam_heatmap_colored = cv2.applyColorMap(
            np.uint8(255 * grad_cam_heatmap_resized), cv2.COLORMAP_JET
        )

        # Superimpose Grad-CAM heatmap on the original image
        output_image = cv2.addWeighted(
            original_image, 0.6, grad_cam_heatmap_colored, 0.4, 0
        )
...
def compute_grad_cam_plus_plus_batch(feature_map_batch, grads_batch):
    # Global average pooling of gradients
    weights_pos = np.maximum(
        0.0, np.sum(grads_batch, axis=(2, 3))
    )  # Positive gradients
    weights_neg = np.maximum(
        0.0, -np.sum(grads_batch, axis=(2, 3))
    )  # Negative gradients
